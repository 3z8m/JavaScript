...
export function WorkrollTable() {
    ...
    // 列番号のリストを取得（重複を除外して昇順にソート）
    const getColumnList = (): string[] => {
        if (!grindingMap) return [];

        const cols: string[] = grindingMap.types.map((t: any) => String(t.column));

        if (grindingMap.default?.column) {
            cols.push(String(grindingMap.default.column));
        }

        return Array.from(new Set(cols)).sort((a, b) => Number(a) - Number(b));
    };
