...
export async function GET(request: Request) {
    ...
    // -----------------------------
    // WC系 grinding_type の roll_id 条件付き集計
    // -----------------------------
    const wcSummaryMap: Record<string, number> = {};

    for (const w of filtered) {
        const grinding = w.grinding_type || "";
        const rollId = w.roll_id || "";

        // grinding_type が WC で始まるものだけ対象
        if (!/^WC/.test(grinding)) continue;

        // roll_id が FWW / IWW / 数字で始まるものだけ対象
        if (!/^(FWW|IWW|\d)/.test(rollId)) continue;

        const key = `${w.roll_type || "未設定"}__${grinding}`;
        const amount = Number(w.grinding_amount) || 0;

        wcSummaryMap[key] = (wcSummaryMap[key] || 0) + amount;
    }

    // WC集計を CSV 行に変換
    const wcSummaryRows = [
        "roll_type,grinding_type,total_grinding_amount(WC条件付き)",
        ...Object.entries(wcSummaryMap).map(([key, total]) => {
            const [roll_type, grinding_type] = key.split("__");
            return `${escapeCSV(roll_type)},${escapeCSV(grinding_type)},${escapeCSV(total)}`;
        })
    ];
    ...
    const csv =
        ...
        [
            ...
            ...summaryRows,             // 集計表
            "",                         // 空行
            "集計結果（WC棚卸資産のみ：FWW/IWW/数字）", // WC集計表のタイトル
            ...wcSummaryRows            // WC集計表
        ].join("\n");
