...
export class MyObj {
    ...
    static obj_secondFloar(scene, x, y, z) {
        ...
        for (let i = 0; i < 15; i++) {
            const poleV = new THREE.Mesh(
                new THREE.CylinderGeometry(0.05, 0.05, 1, 32, 1, false),
                new THREE.MeshStandardMaterial({ color: 0xFFFF00 })
            );
            poleV.position.set(x-5*i+35, y+0.5, z+5);
            scene.add(poleV);
        };
    ...
    static obj_factory2(scene, x, y, z) {
        ...
        // 床
        const plane = new THREE.Mesh(
            new THREE.PlaneGeometry(50, 120, 1, 1), 
            new THREE.MeshPhongMaterial({ color: 0x00AA00 })
        )
        plane.position.set(x+5, y, z)
        plane.rotation.x = -Math.PI / 2
        scene.add(plane)

        // 壁（東）
        const wall_E = new THREE.Mesh(
            new THREE.BoxGeometry(0.2, 16, 120), 
            new THREE.MeshPhongMaterial({ 
                color: 0xFFFFFF,
                opacity: Opacity_wall,          // 透明度
                transparent: true,
        ...
        wall_E.position.set(x-20, y+8, z)
        scene.add(wall_E)

        // 壁（西内）
        const wall_W = wall_E.clone()
        wall_W.position.set(x+20, y+8, z)
        scene.add(wall_W)
        ...
        wall_W2.position.set(x+30, y+8, z)
        scene.add(wall_W2)
        ...
        wall_N.position.set(x+5, y+8, z+60) 
        scene.add(wall_N)
