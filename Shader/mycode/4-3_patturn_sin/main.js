    ...
    const material_sh = new THREE.ShaderMaterial({ 
        ...
        fragmentShader: `
            ...
            void main() { 
                ...
                // UV座標をsin・cosで少しずらす 
                uv.x += sin(uv.y * 6.0 + uTime) * 0.05; 
                uv.y += cos(uv.x * 6.0 + uTime) * 0.05; 
                
                // 歪めたUVでパターンを作る 
                vec2 tiled = fract(uv * 6.0); 
                float stripe = step(0.5, tiled.x); 
                
                vec3 colA = vec3(0.1, 0.3, 0.8); 
                vec3 colB = vec3(0.9, 0.5, 0.1); 
                gl_FragColor = vec4(mix(colA, colB, stripe), 1.0); 
            }
        `, 
    });
