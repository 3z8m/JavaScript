...
async function init() {
    ...
    const material_sh = new THREE.ShaderMaterial({ 
        ...
        fragmentShader: `
            ...
            void main() {
                vec2 uv = vUv - 0.5;                    //中心 (0.5, 0.5) を原点 (0, 0) にする
                float d = length(uv) - 0.3;             // 半径0.3の円のSDF
                float glow = 0.01 / max(abs(d), 0.001); // 輝きの強さを調整
                vec3 col = vec3(0.1, 0.6, 1.0) * glow;  // glow を掛けることで、中心から離れるほど暗くなり、円周が強く発光
